...
SELECT FLOOR(AVG(length)/60) AS 'dur_hours', ROUND(AVG(length) - 60,0) AS dur_minutes
FROM film;
...
SELECT DATEDIFF(MAX(rental_date), MIN(rental_date)) AS number_of_days
FROM rental;
...
SELECT *,
	CASE
	WHEN rental_date IS NOT NULL THEN MONTHNAME(DATE_FORMAT(rental_date, '%y%m%d%H%i%S'))
    END AS rental_month,
	CASE 
	WHEN rental_date IS NOT NULL THEN WEEKDAY(DATE_FORMAT(rental_date, '%y%m%d%H%i%S'))
...
SELECT *,
    ...
    END AS rental_month,
	CASE 
	WHEN WEEKDAY(DATE_FORMAT(rental_date, '%y%m%d%H%i%S'))<5 THEN 'workday'
    ELSE 'weekend'
    /* Note: 0 = Monday, 1 = Tuesday, 2 = Wednesday, 3 = Thursday, 4 = Friday, 5 = Saturday, 6 = Sunday.*/
    END AS day_type
FROM rental;
